...
class RCPGTrainer1(RCSelfTrainer):
  def __init__(self, config: PGConfig, reporter):
    self.config = config
    self.sense_ec = RCSenseEC2()
    self.action_ec= RCActionEC4()
  ...
  def learn_action(self, episode):
    ...
    optimizer = Adam(self.action_model.parameters(), lr=self.config.learning_rate, weight_decay=self.config.weight_decay)
    loss = PGError()
